...
impl StomataSystemMetrics {
    ...
    pub fn fetch(&mut self, fetch_metrics: MetricsToFetch) -> Metrics<'_> {
        match fetch_metrics {
            ...
            MetricsToFetch::SystemResource => Metrics::SystemResource(SystemCollector::fetch(MetricsCategory::Basic, &mut self.system)),
            MetricsToFetch::Process => Metrics::Processes(ProcessData::fetch(&self.system)),
            MetricsToFetch::SingleProcessPid(pid) => Metrics::SingleProcessPid(SingleProcessData::fetch(&mut self.system, pid))
        }
    }
